---
title: "Average Danceability of Music in Latin America"
...
var2 <- "danceable music"

south$danceability <- (as.numeric(south$danceability)*100)
var <- round(south$danceability, 2)
#print(south$africa)
#geojsonio::geojson_write(south, file = "map-7.geojson")
bins <- c(50, 60, 70, 80, 90, 99)
pal <- colorBin("YlOrRd", domain = south$danceability, bins = bins)
...
counting <- 0
naCountries <- c("Falkland Islands", "Nicaragua", "Venezuela", "Guyana", "Suriname", "French Guiana", "Belize", "Jamaica", "Cuba", "Bahamas", "Haiti", "Puerto Rico")
for (i in labels)
{
  counting <- counting + 1
  if(length(i <- grep(paste(naCountries,collapse="|"), i))){
...
#print(labels2)
m %>% addPolygons(fillColor = ~pal(danceability),
  weight = 2,
  opacity = 1,
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px",
    direction = "auto"))%>%
 addLegend(pal = colorBin("YlOrRd", domain = south$danceability, bins = bins, reverse = TRUE), values = ~danceability, opacity = 0.7, title = "Average Danceability of Music<br>by Percentage",
